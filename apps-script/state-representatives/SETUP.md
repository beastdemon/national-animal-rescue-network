# Board of State Representatives — directory setup

This Apps Script web app turns approved rows from the `State Representatives` tab of the Public Website Content Hub into a "meet our representatives" roster with open-seat applications. It never reads the Private Operations Tracker or raw, unreviewed applications.

This is a board-level advocacy role — each state's representative speaks for the network and the dogs in its care to their own state's government. It is not an elected office; don't let the page or the Sheet data get confused with an actual state senator or state representative.
...
8. `Photo URL` and `Bio Or Message` are both optional per row, but the column headers must still exist even if a given row leaves them blank. Only fill these in after the specific representative has separately consented to a public photo and bio — the State Representative / Board Application (see `FORM_BLUEPRINTS.md`) collects that consent at appointment time, not at application time. A filled seat with no photo yet just shows a placeholder icon instead of breaking anything. `Photo URL` must be a public https:// image link; anything else is ignored the same way other https-only fields are.
